...
<title>Tiempo De Entrega a Domicilio (Remisiones)</title>
...
<th>Hora Llegada</th>
<th>Fecha Entrega</th>
<th>H.Recibo Remision Almacen</th> 
<th>Chofer</th>
<th>Vehiculo</th>
...
$fecha2=$_POST['ffin'];
$contat=0;$contft=0;
$query=mysql_query("SELECT remision, fecharem, cliente, horasalida, horaentrega, horallegada, fechaentrega, horarec, chofer, vehiculo, direccion, observ from remision where estatus='ENTREGA DOMICILIO' AND fecharem >= '$fecha1' AND fecharem <= '$fecha2' order by fecharem DESC");
  ...
  while ($array=mysql_fetch_array($query))
  {
    ...
    $hrent=$array[4];
     ...
     //HORARIO DE 8:30 A 1:30
      ...
      if($S!="6")
      { 
        if($hrec>=$hora3 and $hrec<=$hora4) 
        {       
       if($hentrega>strtotime($horalimite2) and strtotime($fechaent)==strtotime($ferem))                           
        {
          $status="<label class='bg1' title='Remisiones recibidas de la 13:31pm a las 18:00pm están en tiempo si se entregan hasta la 13:30pm del día siguiente, sino estaran fuera de tiempo'
          >A TIEMPO</label>";
          $contat=$contat+1;
        }
        else
        { 
         if($hentrega>strtotime($horalimite2) or strtotime($fechaent)>strtotime($fechalimite)) 
         { 
           $status="<label class='bg2' title='Remisiones recibidas de la 13:31pm a las 18:00pm están en tiempo si se entregan hasta las 13:30pm del día siguiente, sino estaran fuera de tiempo'
      ...
      else if($hrec>=$hora1 and $hrec<=$hora2) 
      {
         if($hentrega>$horalimite or strtotime($fechaent)>strtotime($ferem))
           {
             $status="<label class='bg2' title='Remisiones recibidas de las 8:30am a la 13:30pm están en tiempo si se entregan hasta las 18:00pm, del mismo día, sino estaran fuera de tiempo'
             >FUERA DE TIEMPO</label>";
              $contft=$contft+1;   
           }
            else
            {    
            $status="<label class='bg1' title='Remisiones recibidas de las 8:30am a la 13:30pm están en tiempo si se entregan hasta las 18:00pm, del mismo día, sino estaran fuera de tiempo'
            >A TIEMPO</label>";
            $contat=$contat+1;
            }
          }
     ...
     {
       if($hrec>=$hora5 and $hrec<=$hora6)
       {
        if($hentrega>$horalimite or strtotime($fechaent)>strtotime($fechalimite2))                              
        {
           $status="<label class='bg2' title='Remisiones recibidas de 8:30am a 14:00pm estan a tiempo si entregan el lunes antes de las 18:00pm, sino estan fuera de tiempo'
           >FUERA DE TIEMPO</label>";
            $contft=$contft+1;           
        }
        else
        {    
          $status="<label class='bg1' title='Remisiones recibidas de 8:30am a 14:00pm estan a tiempo si entregan el lunes antes de las 18:00pm, sino estan fuera de tiempo'
          >A TIEMPO</label>";
          $contat=$contat+1;
        }
      } 
 ...
 <table id="table">
  ...
  <tr>
   ...
   <tr>
    <td></td>
    <td>
      <button onclick="location.href='fechatiementrega.php'" class="boton_1">Indicador de entregas</button>
    </td>
    <td></td>
  </tr> 
</table>
