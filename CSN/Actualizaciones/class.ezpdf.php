...
function Cezpdf($paper='a4',$orientation='portrait'){
        ...
        if (!is_array($paper)){
                switch (strtoupper($paper)){
                        ...
                        case 'A4': 
                        default:    {$size = array(0,0,595.28,841.89); break;}
...
function ezInsertMode($status=1,$pageNum=1,$pos='before'){
  ...
  switch($status){
    case '1':
      $this->document->addPageTree ($pageNum,(true==$pos));
      break;
    case '0':
      $this->objects->currentNode->close();
      break;
  }
}
// ------------------------------------------------------------------------------
...
//  echo $this->getFontHeight($size)."<br>";
//  echo $height."<br>";
  $lines = explode("\n",$text);
  foreach ($lines as $line){
    $start=1;
...
function alink($info,$internal=0){
  ...
  $lineFactor=0.06; // the thickness of the line as a proportion of the height. also the drop of the line.
  switch($info['status']){
    case 'start':
    case 'sol':
